...
@property (nonatomic, strong) NSDate *requestedOn;
@end
 ...
 */
+ (AccessToken *)tokenWithToken:(NSString *)token requestedOn:(NSDate *)date expiresIn:(long)expiration
{
    AccessToken *accessToken = [[AccessToken alloc] init];
    accessToken.token = token;
    NSDate *cDate = [NSDate date];
    accessToken.requestedOn = cDate;
    accessToken.expiresOn = [cDate dateByAddingTimeInterval:expiration];
    return accessToken;
}
